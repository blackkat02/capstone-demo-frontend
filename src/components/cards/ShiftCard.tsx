...
export function ShiftCard({ shift }: ShiftCardProps) {
  ...
  return (
    <article className="group relative flex flex-col rounded-xl border border-border bg-bg p-4 transition-shadow hover:shadow-[0_8px_30px_-12px_rgba(18,19,26,0.25)] sm:rounded-2xl sm:p-5">
      <div className="flex items-start justify-between">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-bg-muted text-lg sm:h-11 sm:w-11 sm:text-xl">
          {shift.category}
        </div>
        <button
          type="button"
      ...
      <button
        type="button"
        className="mt-4 flex min-h-[44px] items-center justify-between rounded-xl bg-ink px-4 py-2.5 text-sm font-medium text-white transition-colors group-hover:bg-accent"
      >
        Детальніше
